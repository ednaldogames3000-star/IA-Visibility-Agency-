...
const pool = connectionString ? new Pool({ connectionString, max: Number(process.env.DB_POOL_MAX || 3), idleTimeoutMillis: 30_000, connectionTimeoutMillis: 5_000, ssl: process.env.DB_SSL === 'false' ? false : undefined }) : null;
...
export async function listCustomerAudits(customerId, limit = 100) {
  const db = requireDb(); await initStore();
  const { rows } = await db.query('SELECT id,url,score,result,created_at AS "createdAt" FROM audits WHERE customer_id=$1 ORDER BY created_at DESC LIMIT $2', [customerId, safeLimit(limit, 100, 200)]);
  return rows;
}
