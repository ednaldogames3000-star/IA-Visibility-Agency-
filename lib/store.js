...
export async function listAudits(limit = 50) {
  ...
  const { rows } = await db.query('SELECT id,customer_id AS "customerId",url,score,result,created_at AS "createdAt" FROM audits ORDER BY created_at DESC LIMIT $1', [safeLimit]);
  return rows;
}
...
export async function listCustomers(limit = 100) {
  ...
  const { rows } = await db.query('SELECT id,name,company,email,phone,status,notes,created_at AS "createdAt",updated_at AS "updatedAt" FROM customers ORDER BY created_at DESC LIMIT $1', [safeLimit]);
  return rows;
}
...
export async function listLeads(limit = 100) {
  const db = requireDb(); await initStore();
  const safeLimit = Math.min(Math.max(Number(limit) || 100, 1), 200);
  const { rows } = await db.query('SELECT id,name,company,email,phone,source,message,status,created_at AS "createdAt",updated_at AS "updatedAt" FROM leads ORDER BY created_at DESC LIMIT $1', [safeLimit]);
  return rows;
}

export async function addLog(level, event, metadata = {}, requestId = null, actor = null) {
  if (!pool) return;
  const db = pool; await initStore();
  await db.query('INSERT INTO app_logs (level,event,request_id,actor,metadata) VALUES ($1,$2,$3,$4,$5)', [level, event, requestId, actor, JSON.stringify(metadata)]);
}

export async function listLogs(limit = 100) {
  const db = requireDb(); await initStore();
  const safeLimit = Math.min(Math.max(Number(limit) || 100, 1), 200);
  const { rows } = await db.query('SELECT id,level,event,request_id AS "requestId",actor,metadata,created_at AS "createdAt" FROM app_logs ORDER BY created_at DESC LIMIT $1', [safeLimit]);
  return rows;
}
